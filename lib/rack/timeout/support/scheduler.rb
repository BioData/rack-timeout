...
class Rack::Timeout::Scheduler
  ...
  RunEvent = Struct.new(:monotime, :proc) do
    def initialize(*args)
      @cancelled = false
      super(*args)
    end
  ...
  def run_loop!
    ...
    sleep_for = nil
    run = nil
    last_run = fsecs # sleep_for: how long to sleep before next run; last_run: time of last run; run: just initializing it outside of the synchronize scope, will contain events to run now
    loop do # begin event reader loop
      @mx_events.synchronize do
      ...
      run.sort_by(&:monotime).each(&:run!) # run the events scheduled to run now
