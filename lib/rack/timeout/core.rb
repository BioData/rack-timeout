...
module Rack
  class Timeout
    ...
    class RequestTimeoutException < StandardError # This is first raised to help prevent an application from inadvertently catching the above. It's then caught by rack-timeout and replaced with RequestTimeoutError to bubble up to wrapping middlewares and the web server
      include ExceptionWithEnv
    end
    ...
    RequestDetails = Struct.new(
      ...
      def ms(key) # helper method used for formatting values in milliseconds
        format("%.fms", (self[key] * 1000)) if self[key]
      end
    end
    VALID_STATES = [
    ...
    def read_timeout_property(value, default)
      case value
      when nil then read_timeout_property(default, default)
      when false, 0 then false
      else
        (value.is_a?(Numeric) && value.positive?) or raise ArgumentError, "value #{value.inspect} should be false, zero, or a positive number."
        value
      end
    end
    ...
    def call(env)
      ...
      if time_started_wait && wait_timeout
        ...
        seconds_waited          = 0 if seconds_waited.negative? # make up for potential time drift between the routing server and the application server
      ...
      info.timeout = seconds_service_left if !service_past_wait && seconds_service_left && seconds_service_left.positive? && seconds_service_left < service_timeout
      ...
      register_state_change = lambda { |status = :active| # updates service time and state; will run every second
      ...
      timeout.timeout(info.timeout) do # perform request with timeout
        @app.call(env) # boom, send request down the middleware chain
    ...
    RX_NGINX_X_REQUEST_START  = /^(?:t=)?(\d+)\.(\d{3})$/.freeze
    RX_HEROKU_X_REQUEST_START = /^(\d+)$/.freeze
    ...
    def self.notify_state_change_observers(env)
      @state_change_observers.each_value { |observer| observer.call(env) }
    end
  end
end
